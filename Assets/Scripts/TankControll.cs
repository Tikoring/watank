...
public class TankControll : MonoBehaviour
{
    ...
    // 코드를 병합하지 않고 그대로 진행한다면 발생할 수 있는 문제점이
    // PV 값이 여러개 나와서 ViewID 값이 제대로 배정이 안되는 부분임.
    // 그래도 일단 PV를 상속받아서 진행해보자.
    ...
    private void Start()
    {
        ...
        //remainTimeText = GameObject.FindGameObjectWithTag("RemainTimeText").GetComponent<Text>();
    ...
    // 캐릭터 움직임을 Update 에서 관리한다. 

    public void TankControllFunctionInScript()
    {
        if (hit)
        {
            // player control
    ...
    private void Update () {
        ...
        // AssetTank 가 내것이면 컨트롤이 가능하게, 그렇지 않으면 동기화 시켜준다.
        
        // if(PV.IsMine) 부터 내 오브젝트를 photonView 서버로 통신 
